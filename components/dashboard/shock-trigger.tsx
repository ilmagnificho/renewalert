...
export function ShockTrigger({
  ...
  const totalAtRisk = contracts.reduce((acc, contract) => {
    const yearlyAmount =
      contract.cycle === "monthly" ? contract.amount * 12 : contract.amount;
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="pt-6 space-y-2">
          ...
          <div className="text-5xl sm:text-7xl font-black text-white tracking-tighter">
            👉 {formatCurrency(totalAtRisk, contracts[0]?.currency || "KRW")} /{" "}
            {isEnglish ? "year" : "연"}
          </div>
        </div>
        ...
        <div className="pt-8 border-t border-white/5 mt-8">
          <p className="text-zinc-400 text-sm sm:text-base font-bold leading-relaxed">
            {isEnglish
              ? "Reviewing before auto-renewal can prevent this spend."
              : "자동 갱신 전에 검토하면 이 비용은 막을 수 있습니다."}
          </p>
